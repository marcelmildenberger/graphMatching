...
class TSHEncoder(Encoder):
    ...
    def __init__(
        ...
        """Initialize the hash-function and hash-column parameters for the encoder.
        ...
        if rand_mode not in ("PNG", "SHA"):
            raise ValueError(f"Invalid rand_mode '{rand_mode}'. Must be one of ['PNG', 'SHA']")
        ...
        self.salt = ''.join(random.choice(string.ascii_letters) for _ in range(32)) if secret is None else secret
    ...
    def enc(self, data):
        """Apply column-based vector hashing to the given q-gram input.

           Input arguments:
             - q_gram_set  The set of q-grams (strings) to encode.
    ...
    def encode_and_compare(self, data, uids, metric, sim=True, store_encs=False):
        ...
        del cache
        gc.collect()
        result = np.zeros((numinds, 3), dtype=np.float32)
